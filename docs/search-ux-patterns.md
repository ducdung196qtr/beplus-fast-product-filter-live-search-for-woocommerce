...
> Frontend and editor patterns for live product search in **BePlus Smart Search**. Use when building blocks, `view.source.ts`, or accessibility behavior.
...
| Key | Purpose |
|-----|---------|
| `restUrl` | Base REST URL (`beplus-smart-search/v1`) |
...
```html
<div class="beplus-smart-search beplus-smart-search--inline|sidebar"
...
- `beplus-smart-search--loading` — fetch in progress
- `beplus-smart-search--ready` — initialized
- BEM: `beplus-smart-search__*` for elements
...
3. `collectFilters(form)` → query params for `GET /beplus-smart-search/v1/products`.
...
Facet counts: `GET /beplus-smart-search/v1/facets` with current filter params.

---

## 4. Editor preview

Use **ServerSideRender** with block `beplus-smart-search/advanced-woo-search` inside the block `edit.tsx` inspector preview. Matches PHP `render.php` output without duplicating markup in JavaScript.
...
| Filter | Purpose |
|--------|---------|
| `beplus_smart_search_rest_products_args` | Modify product query before fetch |
| `beplus-smart-search/search.query` | Modify parsed search query object |
| `beplus-smart-search/search.results` | Modify REST response payload |
